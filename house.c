...
void initHouse(HouseType* house){
    initList(&house->allHunters);
    initList(&house->rooms);
    initList(&house->sharedEvidenceCollected);
}

void freeHouse(HouseType* house){
    NodeType* currNode = house->rooms.head;
    while(currNode != NULL){
        freeRoom(currNode->data);
        currNode = currNode->next;
    }
    freeListNodes(&house->rooms);
    freeListData(&house->allHunters);
    freeListNodes(&house->allHunters);
    freeListData(&house->sharedEvidenceCollected);
    freeListNodes(&house->sharedEvidenceCollected);
}